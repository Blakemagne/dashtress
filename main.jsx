...
const MattressDashboard = () => {
  ...
  const OnboardingScreen = () => {
    ...
    const loadDemoData = async () => {
      ...
      try {
        ...
        const response = await window.fs.readFile('20250330_reportWrittenSalesDollars.xlsx');
        ...
        // If valid, store the file data
        setFileData(jsonData);
        setUploadedFile({ name: 'Demo: 20250330_reportWrittenSalesDollars.xlsx' });
        setShowOnboarding(false);
        
      } catch (err) {
        console.error("Error loading demo data:", err);
        setUploadError("Demo data file not found. Please upload your own Excel file.");
      } finally {
          ...
          <div className="mt-6 text-center">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-gray-50 text-gray-500">Or try with demo data</span>
              </div>
            </div>
            
            <button
              onClick={loadDemoData}
              disabled={loadingDemo || isValidating}
              className="mt-4 px-6 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center mx-auto"
            >
              {loadingDemo ? (
                <>
            ...
            <p className="text-xs text-gray-500 mt-2">
              Explore the dashboard with sample mattress sales data
            </p>
          </div>
          ...
          <div className="mt-8 bg-blue-50 rounded-lg p-6">
            ...
            <div className="space-y-2 text-sm text-blue-800">
              ...
              <p className="mt-2 text-xs text-blue-700 font-medium">
                Not sure about the format? Try the demo mode first to see how the dashboard works!
              </p>
            </div>
          </div>
        </div>
  ...
  const DashboardHeader = () => {
    const isDemo = uploadedFile?.name?.startsWith('Demo:');
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm text-gray-600">
            ...
            {uploadedFile?.name?.replace('Demo: ', '') || 'No file loaded'}
          </div>
          <button
            onClick={() => {
